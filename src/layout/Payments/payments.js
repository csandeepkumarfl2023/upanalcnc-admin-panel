...
export default function Payments() {
  ...
  const submitHandler = async () => {
    ...
    let res = await paymentsservice.postPayments(currentData)
    setName('')
    setDescription('')
    setType('')
    getData()
  ...
  const editBtnHandler = async() => {
    ...
    let res = await paymentsservice.putPayments(updatedData, updateId)
    getData()
    setEditModal(false)
    setLoading(true)
    setTimeout(function () {
  ...
  const deleteHandler = async() => {
    let res = await paymentsservice.deletePayments(updateId)
    getData()
    setEditModal(false)
    setLoading(true)
    setTimeout(function () {
  ...
  const getData = async () => {
    let res = await paymentsservice.getAllPayments()
    setData(res)
  }
            ...
            <CCard>
              <CCardBody>
                <CRow>
                  ...
                  <CCol xs="1">
                    <CButton color="info" onClick={addBtnHandler} className="mr-1">New</CButton>
                  </CCol>
                </CRow>
                <CDataTable
                  items={data}
